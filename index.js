...
const { PORT, QUICKNODE_HTTP_ENDPOINT, PRIVATE_KEY, CONTRACT_ADDRESS } =
  process.env;
const port = PORT;
...
async function mintNFT(address, URI) {
  try {
    ...
    const txn = await contractWithSigner.mintNFT(address, URI, {
      gasPrice: gasFee,
    });

    console.log(
      "...Submitting mint transaction with gas price of:",
...
async function transferNFT(fromAddress, toAddress, tokenId) {
  try {
    ...
    const txn = await contractWithSigner.transferFrom(
      fromAddress,
      toAddress,
      tokenId,
      { gasPrice: gasFee }
    ...
    console.log(
      "...Submitting transfer transaction with gas price of:",
      ethers.utils.formatUnits(gasFee, "gwei")
    );
    let receipt = await txn.wait();
...
app.post("/mint", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .send({ error: "Error minting NFT.", details: error.message });
  }
});
...
app.post("/transfer", async (req, res) => {
  ...
  if (!fromAddress || !toAddress || !tokenId) {
    return res
      .status(400)
      .send("From address, to address, and token ID are required.");
  }
  try {
    ...
    res
      .status(500)
      .send({ error: "Error transferring NFT.", details: error.message });
  }
});

app.get("/", (req, res) => {
  res.send("NFT minting service running.");
});
